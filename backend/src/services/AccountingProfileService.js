...
export function createAccountingProfileService({
  ...
  return {
    ...
    async ensureForProject(userId, projectId) {
      ...
      const existing = await accountingProfileRepository.findByCompanyId(company.id);
      if (existing) return existing;
      return accountingProfileRepository.create({ companyId: company.id });
    },

    async getForProject(userId, projectId) {
      const company = await companyService.getForProject(userId, projectId);
      const profile = await accountingProfileRepository.findByCompanyId(company.id);
    ...
    async updateForProject(userId, projectId, fields) {
      ...
      return accountingProfileRepository.update(profile.id, fields);
    },

    async markTransmitted(userId, projectId) {
      const profile = await this.ensureForProject(userId, projectId);
      return accountingProfileRepository.update(profile.id, {
        status: 'transmis',
    ...
    async buildDossier(userId, projectId) {
      ...
      const profile =
        (await accountingProfileRepository.findByCompanyId(company.id)) ??
        (await accountingProfileRepository.create({ companyId: company.id }));
